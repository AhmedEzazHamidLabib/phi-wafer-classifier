...
# ── 2. AUGMENTATION ───────────────────────────────────────────────────────────
# rotation and flipping safe for all classes
# noise only for symmetric patterns — not Scratch
# Scratch linear structure is too fragile for noise injection
...
# velocity constants — self-calibrating, no manual tuning
ALPHA_MAX    = 3.0
BETA_MIN     = 0.3
VEL_SCALE    = 5.0
VEL_CLAMP_LO = 0.5
...
def compute_amplifier(true_labels, phi_vector, phi_vel):
    ...
    for i in range(len(true_labels)):
        ...
        # improving fast → back off
        # stuck or falling → push harder
        vf = max(VEL_CLAMP_LO,
...
aug_macro = report['macro avg']['f1-score']
d_macro   = aug_macro - b_macro

print(f"\n{'Macro':<12} {b_macro:>7.4f} {v1_macro:>7.4f} "
      f"{v4_macro:>7.4f} {v5_macro:>7.4f} {aug_macro:>7.4f} "
      f"{'↑' if d_macro>0 else '↓'}{abs(d_macro):.4f}")

with open("phi_v4_aug_report.json", "w") as f:
    json.dump(report, f, indent=2)

torch.save(model.state_dict(), "phi_v4_aug_cnn.pt")
print("\nSaved phi_v4_aug_cnn.pt and phi_v4_aug_report.json")
